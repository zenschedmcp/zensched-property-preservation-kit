...
- **Photo stamp is opt-in per field.** Compliance photo fields in the kit set `"stamp_photos": true` so the server burns readable date, time, and GPS onto exported JPEGs from capture metadata. Each image also stores `capture_lat`, `capture_lng`, `capture_ts`, and `capture_source` on the record. Gallery picks without EXIF may stamp date/time only. The GPS **punch** is separate: whether the inspector was inside the geofence. You still upload to vendor portals in their app.
